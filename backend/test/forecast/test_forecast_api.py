...
2021,2020-11-13,0000,Test League,Always Wins,Always Loses,65.59,39.99,0.7832,0.0673,0.1495,2.58,0.62,77.1,28.8,90,0,0.49,0.45,1.05,0.75,3.15,0.0
2021,2020-11-14,0000,Test League,Other,Another,65.59,39.99,0.7832,0.0673,0.1495,2.58,0.62,77.1,28.8,1,1,0.49,0.45,1.05,0.75,3.15,0.0"""
    ...
    def test_forecast_full_simulator_model(self) -> None:
        response = self.test_client.get(
            '/forecast?home_name=Always+Loses&away_name=Always+Wins&league=Test+League&model_name=Full+simulator'
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json(), {
            'fixture': {
                'away_team': {'name': 'Always Wins'},
                'home_team': {'name': 'Always Loses'},
                'league': 'Test League'
            },
    ...
    def test_forecast_home_model(self) -> None:
        response = self.test_client.get(
            '/forecast?home_name=Always+Loses&away_name=Always+Wins&league=Test+League&model_name=Home'
        )
